...
const DEFAULT_SEASONAL_PRICE_RANGES = Object.freeze([
  ...
  },
  {
    ...
    nightly: 134,
    weeklyTotal: 938,
    note: "perioada de varf, potrivita pentru vacanta la mare",
  },
  {
    label: "August",
    month: 8,
    ...
    nightly: 134,
    weeklyTotal: 938,
    note: "luna de varf, disponibilitatea se confirma rapid",
  },
  {
    label: "Septembrie",
    month: 9,
...
const DEFAULT_ACCOMMODATIONS = [
  {
    ...
    maxAdults: 2,
    maxChildren: 1,
    summary: "Studio luminos cu pat dublu, baie privata, balcon si acces rapid spre plaja, potrivit si pentru 1 copil pana la 12 ani.",
    highlights: [
      "pat dublu si decor modern",
      "baie privata si aer conditionat",
      "accepta 1 copil pana la 12 ani",
    ],
  },
  {
    id: "studio-deluxe",
...
const GALLERY_FILES = [
  "afroditi-studios-paralia-katerinis-noapte-exterior.jpg",
  "i-love-paralia-katerinis-plaja.jpg",
  "plaja-paralia-katerinis-afroditi-studios.jpg",
  "promenada-paralia-katerinis-seara-afroditi-studios.jpg",
  "apus-paralia-katerinis-ponton-afroditi-studios.jpg",
  "palmieri-paralia-katerinis-aproape-de-plaja.jpg",
  "muntele-olimp-vazut-din-paralia-katerinis.jpg",
...
const GALLERY_IMAGE_ALT_TEXT = {
  ...
  "afroditi-studios-paralia-katerinis-noapte-exterior.jpg":
    "Exteriorul AFRODITI Studios Grigoriu iluminat seara in Paralia Katerinis",
  "i-love-paralia-katerinis-plaja.jpg":
    "Semnul I love Paralia langa plaja din Paralia Katerinis",
  "plaja-paralia-katerinis-afroditi-studios.jpg":
    "Plaja din Paralia Katerinis cu palmieri si vedere spre mare",
  "promenada-paralia-katerinis-seara-afroditi-studios.jpg":
    "Promenada animata seara in Paralia Katerinis aproape de AFRODITI Studios",
  "apus-paralia-katerinis-ponton-afroditi-studios.jpg":
    "Apus peste pontonul din Paralia Katerinis, Grecia",
  "palmieri-paralia-katerinis-aproape-de-plaja.jpg":
    "Palmieri si zona verde aproape de plaja in Paralia Katerinis",
  "muntele-olimp-vazut-din-paralia-katerinis.jpg":
    "Muntele Olimp vazut din zona Paralia Katerinis",
...
const EXTRACTED_REVIEWS = [
  ...
  },
  {
    ...
    title: "Exceptional",
    room: "Apartament studio cu 1 pat dublu",
    stay: "6 nopti | calatorind singur",
    quote: "Das Apartment ist ganz neu und gibt es Fahrstuhl. Der Gastgeber ist super nett und zuverlässig. Restaurants, Strände und alles, was man braucht, sind nur 100 Meter entfernt, und trotzdem ist die Lage sehr ruhig. Parkplätze sind leicht zu finden. Ich kann die Unterkunft mit großer Freude weiterempfehlen!",
    tags: ["apartament nou", "gazda de incredere", "aproape de plaja"],
  },
  {
    name: "Trajstarevic",
    country: "Germania",
...
const TRANSLATIONS = {
  en: {
    ...
    "Social si email": "Social and email",
    "Cadre reale si mici update-uri din atmosfera proprietatii.": "Real moments and small updates from the property's atmosphere.",
    "Cadre reale, mici update-uri si un contact scris direct.": "Real moments, small updates and a direct written contact.",
    "Trimite email la contact@afroditistudiosgrigoriu.com": "Send email to contact@afroditistudiosgrigoriu.com",
  ...
  el: {
    ...
    "Social si email": "Social και email",
    ...
    "Cadre reale, mici update-uri si un contact scris direct.": "Πραγματικές εικόνες, μικρές ενημερώσεις και άμεση γραπτή επικοινωνία.",
    "Trimite email la contact@afroditistudiosgrigoriu.com": "Στείλτε email στο contact@afroditistudiosgrigoriu.com",
...
const bookingChildAgeField = $("#booking-child-age-field");
const bookingChildAgeInput = $("#booking-child-age");
...
function getStayPriceEstimate(checkIn, checkOut) {
  const days = expandDateRange(checkIn, checkOut);
  const ranges = days.map(getSeasonalPriceRangeForDate);

  if (!days.length || ranges.some((range) => !range)) {
    return {
      nights: days.length,
      total: null,
      hasPublishedRate: false,
    };
  }
  ...
  return {
    ...
    minNightly,
    maxNightly,
    isMixedRate: minNightly !== maxNightly,
  };
}

function formatNightlyRate(estimate) {
  if (!estimate?.hasPublishedRate) {
    return "tarif confirmat direct";
  }
  ...
  if (bookingGuestHint) {
    bookingGuestHint.textContent =
      limits.maxChildren > 0
        ? `Maxim ${maxSummary} pentru aceasta unitate. Copilul trebuie sa aiba maximum 12 ani.`
        : `Maxim ${limits.maxAdults} ${limits.maxAdults === 1 ? "adult" : "adulti"} pentru aceasta unitate.`;
  }
  if (bookingChildAgeField && bookingChildAgeInput) {
    const needsChildAge = selection.children > 0;
    bookingChildAgeField.hidden = !needsChildAge;
    bookingChildAgeInput.required = needsChildAge;
    bookingChildAgeInput.disabled = !needsChildAge;
    bookingChildAgeInput.max = "12";
    bookingChildAgeInput.min = "0";
    if (!needsChildAge) {
      bookingChildAgeInput.value = "";
      bookingChildAgeInput.setCustomValidity("");
    }
  }
...
function syncBookingDateFields(options = {}) {
  ...
  if (!checkIn) {
    updateBookingPriceNote(`Selecteaza un sejur de minimum ${MINIMUM_BOOKING_NIGHTS} nopti pentru a vedea estimarea in euro.`);
    if (!keepStatus) {
  ...
  if (!checkOut) {
    const minimumStayEstimate = getStayPriceEstimate(checkIn, minimumCheckoutDate);
  ...
  const estimate = getStayPriceEstimate(checkIn, checkOut);
  if (!estimate.hasPublishedRate) {
    updateBookingPriceNote("Tariful pentru perioada aleasa se confirma direct, in functie de disponibilitate.");
  } else {
    updateBookingPriceNote(
...
function setVisitorModalOpen(isOpen, { locked = false } = {}) {
  ...
  if (isOpen) {
    window.requestAnimationFrame(() => {
      const modalShell = visitorPreferencesModal.querySelector(".visitor-modal-shell");
      if (modalShell instanceof HTMLElement) {
        modalShell.tabIndex = -1;
        modalShell.scrollTop = 0;
        if (COMPACT_AVAILABILITY.matches) {
          modalShell.focus({ preventScroll: true });
          return;
        }
      }
      if (visitorSaveEssential) {
...
async function handleBookingSubmit(event) {
  ...
  const childAgeText = String(data.get("childAge") ?? "").trim();
  ...
  if (
    ...
    && (!/^\d{1,2}$/.test(childAgeText) || !Number.isInteger(childAge) || childAge < 0 || childAge > 12)
  ) {
    setStatus("booking-status", "Completeaza varsta copilului, intre 0 si 12 ani.", "error");
    bookingChildAgeInput?.focus();
    return;
  }
  ...
  try {
    bookingResponse = await fetchJson("/api/booking-request", {
      ...
      body: JSON.stringify({
        ...
        childAge: guestSelection.children > 0 ? childAge : null,
