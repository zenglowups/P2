...
const SITE_POLICY_VERSION = "2026-05-07-privacy-consent";
...
function pruneSiteState(state, nowIso = new Date().toISOString()) {
  const normalized = normalizeSiteState(state);
  const cutoffIso = getCutoffIso(VISITOR_RETENTION_DAYS);

  normalized.visitors = Object.fromEntries(
    Object.entries(normalized.visitors).filter(([, visitor]) => {
      const lastSeenAt = String(visitor?.lastSeenAt || "").trim();
      return lastSeenAt && lastSeenAt >= cutoffIso;
    }),
  );

  normalized.dailyStats = Object.fromEntries(
    Object.entries(normalized.dailyStats).filter(([dateKey]) => `${dateKey}T00:00:00.000Z` >= cutoffIso),
  );

  normalized.updatedAt = normalizeIsoDate(nowIso, nowIso);
  return normalized;
}
...
async function readLocalSiteState() {
  ...
  try {
    ...
    return pruneSiteState(JSON.parse(raw));
  } catch (error) {
...
async function writeLocalSiteState(state) {
  await ensureLocalDataDir();
  const normalized = pruneSiteState(state);
...
async function readBlobSiteState() {
  ...
  try {
    return pruneSiteState(JSON.parse(text));
  } catch {
...
async function readSiteState() {
  ...
  return pruneSiteState(memoryState);
}
...
async function writeSiteState(state) {
  ...
  memoryState = pruneSiteState(state);
  return memoryState;
}
...
function buildVisitorPayload(visitor, overrides = {}) {
  return {
    ...
    preferencesSaved: !!visitor.preferences.termsAcceptedAt,
...
function syncVisitorPreferences(visitor, nowIso, { analyticsEnabled = false, source = "manual" } = {}) {
  const previousPreferences = normalizePreferences(visitor.preferences);
  const nextSavedAt = previousPreferences.savedAt || nowIso;
  const nextSource = sanitizeSource(source || previousPreferences.source || "manual");
...
export async function saveVisitorPreferences({ headers, socketAddress, pathname, source, analyticsEnabled = false }) {
  const nowIso = new Date().toISOString();
  ...
  const isNewVisitor = !state.visitors[visitorId];

  syncVisitorPreferences(visitor, nowIso, {
    analyticsEnabled: !!analyticsEnabled,
    source: source || previousPreferences.source || "manual",
  });
  visitor.lastSeenAt = nowIso;
  visitor.lastPath = sanitizePathname(pathname);
