...
import { sendBookingNotifications } from "./mailer.mjs";
...
const ACCOMMODATIONS = new Map([
  ["studio-double", { name: "Apartament studio cu 1 pat dublu", maxAdults: 2, maxChildren: 1 }],
  ["studio-deluxe", { name: "Studio Deluxe", maxAdults: 2, maxChildren: 1 }],
]);
...
function validateBookingPayload(payload) {
  ...
  const accommodation = ACCOMMODATIONS.get(accommodationId);
  ...
  const rawChildAge = String(payload.childAge ?? "").trim();
  const childAge = rawChildAge === "" ? null : parseInteger(rawChildAge, -1);
  ...
  if (
    ...
    || adultCount < 1
    || adultCount > accommodation.maxAdults
    || childCount < 0
    || childCount > accommodation.maxChildren
  ) {
    throw genericValidationError();
  }
  if (
    childCount > 0
    && (!/^\d{1,2}$/.test(rawChildAge) || !Number.isInteger(childAge) || childAge < 0 || childAge > 12)
  ) {
    throw genericValidationError();
  }
  ...
  const childAgeLabel = childCount ? ` (${childAge} ${childAge === 1 ? "an" : "ani"})` : "";
  const guestCount = `${adultCount} ${adultCount === 1 ? "adult" : "adulti"}${childCount ? ` + ${childCount} ${childCount === 1 ? "copil" : "copii"}${childAgeLabel}` : ""}`;
...
export async function handlePublicBookingRequest({ payload, headers, socketAddress = "" }) {
  ...
  try {
    ...
    try {
      emailResult = await sendBookingNotifications(bookingPayload);
    } catch (mailError) {
      console.error("booking email dispatch failed", mailError);
    }
    ...
    await logSecurityEvent({
      ...
      metadata: {
        ...
        ownerEmailSent: !!emailResult.ownerSent,
    ...
    return {
      ...
      payload: {
        ok: true,
        message: "Cererea a fost trimisa. Ti-am trimis un email de confirmare.",
      },
    };
  } catch (error) {
