...
export function getStorageMode() {
  return String(process.env.BLOB_READ_WRITE_TOKEN || "").trim() ? "vercel-blob" : "local-file";
}
...
async function readOwnerState() {
  return getStorageMode() === "vercel-blob" ? readBlobState() : readLocalState();
}

async function writeOwnerState(state) {
  return getStorageMode() === "vercel-blob" ? writeBlobState(state) : writeLocalState(state);
}
