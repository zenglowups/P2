...
const DEFAULT_PRICING_RANGES = Object.freeze([
  {
    label: "1-15 iunie",
    month: 6,
    startDay: 1,
...
function sanitizePricingText(value, maxLength) {
  return String(value || "")
    .replace(/\s+/g, " ")
    .trim()
    .slice(0, maxLength);
...
function getDefaultPricingLabel(month, startDay, endDay) {
  ...
  const monthName = monthNames[month] || "Perioada";
  return startDay === 1 && endDay >= 28 ? monthName : `${startDay}-${endDay} ${monthName.toLowerCase()}`;
}

function normalizePricingRange(raw, index = 0) {
...
export function normalizePricingSettings(raw) {
  const sourceRanges = Array.isArray(raw?.ranges) ? raw.ranges : Array.isArray(raw) ? raw : [];
  const ranges = sourceRanges.map(normalizePricingRange).filter(Boolean).slice(0, 12);
  const normalizedRanges = ranges.length
    ? ranges
    : DEFAULT_PRICING_RANGES.map((range, index) => normalizePricingRange({ ...range, order: index })).filter(Boolean);

  return {
    ranges: normalizedRanges.sort((a, b) => a.month - b.month || a.startDay - b.startDay || a.order - b.order),
    updatedAt: sanitizePricingText(raw?.updatedAt, 40),
  };
}
...
function normalizeOwnerState(raw, fallbackState = null) {
  ...
  return {
    ...
    pricingSettings: normalizePricingSettings(raw?.pricingSettings || fallbackState?.pricingSettings),
...
export async function updateOwnerCredentials({ headers, currentPassword, nextUsername, nextPassword }) {
  ...
  return {
    payload: {
      ...
      pricingSettings: savedState.pricingSettings,
      overrides: savedState.overrides,
      storageMode: getStorageMode(),
    },
...
export async function updatePublicPricingSettings({ headers, ranges }) {
  const state = await readOwnerState();
  assertAuthenticated(headers, state);

  const submittedRanges = (Array.isArray(ranges) ? ranges : [])
    .map(normalizePricingRange)
    .filter(Boolean)
    .slice(0, 12)
    .sort((a, b) => a.month - b.month || a.startDay - b.startDay || a.order - b.order);

  if (!submittedRanges.length) {
    throw new HttpError(400, "Adauga cel putin o perioada cu tarif valid.");
  }
