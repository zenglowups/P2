...
function createPutExtraChecks(methodName) {
  return function extraChecks(options) {
    if (!options.token.startsWith("vercel_blob_client_")) {
      throw new (0, _chunkWCA6LLLUcjs.BlobError)(`${methodName} must be called with a client token`);
    }
    if (
...
var upload = _chunkWCA6LLLUcjs.createPutMethod.call(void 0, {
  allowedOptions: ["contentType"],
  extraChecks(options) {
...
async function handleUpload({
  ...
  var _a, _b, _c, _d;
  const resolvedToken = _chunkWCA6LLLUcjs.getTokenFromOptionsOrEnv.call(void 0, { token });
  ...
  switch (type) {
    ...
    case "blob.upload-completed": {
      ...
      if (!signature) {
        throw new (0, _chunkWCA6LLLUcjs.BlobError)("Missing callback signature");
      }
      ...
      if (!isVerified) {
        throw new (0, _chunkWCA6LLLUcjs.BlobError)("Invalid callback signature");
      }
    ...
    default:
      throw new (0, _chunkWCA6LLLUcjs.BlobError)("Invalid event type");
  }
}
async function retrieveClientToken(options) {
  ...
  if (!res.ok) {
    throw new (0, _chunkWCA6LLLUcjs.BlobError)("Failed to  retrieve the client token");
  }
  try {
...
async function generateClientTokenFromReadWriteToken({
  ...
  const readWriteToken = _chunkWCA6LLLUcjs.getTokenFromOptionsOrEnv.call(void 0, { token });
  ...
  if (!securedKey) {
    throw new (0, _chunkWCA6LLLUcjs.BlobError)("Unable to sign client token");
  }
